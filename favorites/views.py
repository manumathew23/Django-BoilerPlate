...
from .utilities import fetch_data, get_customization, add_additional_fields
...
class CommonView(APIView):
    ...
    def get(self, request, category, item_id=None):
        """
        Display a list of `<category>` items (planets / films / people / starships)
        ...
        # Fetch swapi data for the category items or details of item_id
        data = fetch_data(category, item_id)

        # Get custom name and is_favorite mapping for the category for the user
        custom_map = get_customization(user_id, category, search_key)

        for item in data:
            item_dict = dict()
            # id of an item is available as the last part of url
            item_dict["name"], item_dict["is_favorite"] = custom_map.get(
                int(item.get("url").split("/")[-2]),
                (item.get("name"), False)
            )

            if search_key and search_key not in item_dict["name"]:
                continue

            response.append(add_additional_fields(item_dict, item, category))
